...
class Usuario{
    ...
    public function login($usuario, $senha){
        ...
        $queryConsult = "SELECT id, nome_usuario, senha_usuario FROM tabela_login WHERE nome_usuario = :nomeLogin AND senha_usuario = :senhaLogin LIMIT 1";

        $resultado = $conexao_pdo->prepare($queryConsult);
        $resultado->bindParam(":nomeLogin", $usuario);
        $resultado->bindParam(":senhaLogin",md5($senha));
        $resultado->execute();
    ...
    public function cadastro($nomeCad, $sobrenome, $email, $telefone, $data_nascimento, $senhaCad){
        ...
        $queryCad = "SELECT id FROM tabela_login WHERE email = :email";


        $procuraLogin = $conexao_pdo->prepare($queryCad);
